...
To divide the work of rendering the planet into equal parts across the world the XYZ coordinate schema described in \autoref{part1_xyz_coordinates} is used to divide the planet into several subpyramids.
...
To support removed subpyramids in tile server (or even client side rendering) implementations need to be able to backfill missing data in higher zoom levels with data from lower zoom levels.
